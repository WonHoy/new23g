...
BOEING_METHOD = 'Boeing'
DOUGLAS_METHOD = 'Swift_Douglas'
AIRBUS_METHOD = 'Huth_Airbus'


class PlatePoint:
    def __init__(self, plate_id: int, thickness: float, width: float, coord_y: int, material_id: str, plate_E: float):
        self.id = plate_id
...
class Node:
    def __init__(self, joint_info: dict, index: int):
        ...
        for i in range(self.plates_qty):
            ...
            coord_y = plates_dict[plate_id][1]
            material_id = plates_dict[plate_id][0]
            plate_E = plates_dict[plate_id][2]
            plate = PlatePoint(plate_id, thickness, width, coord_y, material_id, plate_E)
...
class JointInputData:
    def __init__(self, joint_dict):
        ...
        self.method = DOUGLAS_METHOD  # By default
        if method == 'swift':
            self.method = DOUGLAS_METHOD
        if method == 'boeing':
            self.method = BOEING_METHOD
        if method == 'huth':
            self.method = AIRBUS_METHOD
        ...
        # Plate ids
        self.plates_ids = plates_ids_sorted_by_y(joint_dict)
...
class PlateElement():
    def __init__(self, plate_id, node1_id, node2_id, E, W, t1, t2, coord1_x, coord2_x, coord_y: float):
        nodes_data = sorted([(coord1_x, node1_id, t1), (coord2_x, node2_id, t2)], key=itemgetter(0))
        [(coord1_x, node1_id, t1), (coord2_x, node2_id, t2)] = nodes_data
    ...
    def calculate_stiffness(self) -> float:
        [t1, t2] = self.thicknesses
        if t1 == t2:
            stiffness = self.E * self.W * t1 / self.length
        else:
            tmax = max(t1, t2)
            tmin = min(t1, t2)
            stiffness = self.E * self.W * (tmax - tmin) / (self.length * log(tmax / tmin))
        return stiffness
    ...
    def set_displacements(self, coord1_x, disp1, coord2_x, disp2):
        ...
        self.end_load = self.delta_x * self.stiffness
        [t1, t2] = self.thicknesses
        stress1 = self.end_load / (self.W * t1)
        stress2 = self.end_load / (self.W * t2)
...
class FastenerElement:
    ...
    def set_displacements(self, plate1_id, coord1_y, disp1, plate2_id, coord2_y, disp2):
        ...
        self.ids_sorted_by_y = [plates_ids[0][1], plates_ids[1][1]]
        self.delta_x = nodes_data[1][1] -nodes_data[0][1]
        self.end_load = self.delta_x * self.stiffness

    def get_end_load(self, plate_id: int)->float:
...
class ConnectedElements:
    ...
    def summary_at(self, node_id: int, plate_id: int) -> tuple:
        ...
        for f_elm in connected_fastener_elms:
            # print(f_elm)
            f_summ = f_summ + f_elm.get_end_load(plate_id)
        ...
        jarfall_effective_stress = 0.569 * fbr + ft_by
    ...
    def stress_summary_at(self,node_id: int, plate_id: int) ->tuple:
        pass
...
if __name__ == '__main__':
    ...
    test_joint_info = {'method': 'boeing', 'nodes': [
        ...
        {'id': 5, 'coord_x': 4, 'plates_id': [3], 'plates_th': [0.121], 'plates_width': [1], 'plates_area': [0.121],
         ...
         'plates': {1: ['al', 1000, 10500000], 2: ['al', 2000, 10500000], 3: ['al', 3000, 10500000]},
