...
class FastenerModel:
    def __init__(self, node: Node, connected_plates_ids: list, method_name: str):
        ...
        self.Df = self.node.fastener_dia
        self.Dh = self.node.hole_dia
        self.Gf = self.node.fastener_Gb
        # Fastener quantity
        self.Qtyf = self.node.fasteners_qty

        # TODO set fastener type explicitly
        if fastener_id[0:4] == 'BACB':
            self.ftype = 'bolt'
        else:
            self.ftype = 'rivet'
        ...
        for f_elm_id in self.fastener_elements_ids:
            ...
            if self.ftype == 'rivet':
                ...
                diameter = self.Df
            f_elm = FastenerElement2(node.id, f_elm_id, stiffness, diameter, node.coord_x)
    ...
    def calculate_stiffness(self, plate1_id, plate2_id, method_name) -> float:
        ...
        for pair in adjacent_plates_pairs:
            ...
            if method_name == BOEING_METHOD:
                ...
                flexibilities.append(flexibility * self.Qtyf)

            if method_name == DOUGLAS_METHOD:
                flexibilities.append(self.swift_douglas_flexibility(pl1_id, pl2_id) * self.Qtyf)

            if method_name == AIRBUS_METHOD:
                flexibilities.append(self.huth_airbus_flexibility(pl1_id, pl2_id) * self.Qtyf)

        stiffness = 1 / sum(flexibilities)
        return stiffness
